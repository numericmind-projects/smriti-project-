...
library(dplyr)
```
#Listing 6 of Protocol Deviations Leading to Exclusion from the Per-Protocol Population 

```{r}
addv <- read_sas("E:/Numeric mind/adam/New_Study-master/New_Study-master/adam/addv.sas7bdat", NULL)
addv1<- addv %>% filter(ITTEFL=="Y" & EXCLPPFL== "Y" & DTYPE!= "PHANTOM") %>% 
  select(SITEID,USUBJID,TRT01P,ASTDT,ASTDY,ATERM,ADECOD,ACAT)
list <- addv1 %>% 
...
title <- c("Listing 6 <br/>Listing of Protocol Deviations Leading to Exclusion from the Per-Protocol Population <br/> <br/>"=5 )
